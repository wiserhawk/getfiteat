package com.indhawk.getfiteat.vendor.manager.service;
...
import com.indhawk.getfiteat.vendor.manager.util.Encryptor;
import com.indhawk.getfiteat.vendor.manager.util.FormatUtil;
...
public class VendorManagerService {
	...
	@Value("${vendor.id.prefix}")
	private String VENDOR_ID_PREFIX;
	...
	private synchronized String getNextVendorId() {
		...
		if (vendorDO == null) {
			// First Vendor
			return  VENDOR_ID_PREFIX + "-" + 1;
		}
		// next vendor
		String lastVendorID = vendorDO.getVendorId();
		Integer nextID = Integer.parseInt(lastVendorID.split("-")[1]) + 1;
